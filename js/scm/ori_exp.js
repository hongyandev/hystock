...
function refresh(id) {
    ...
    jq('#tabs').tabs('update', {
        tab: tab,
        options: {
            content:'<iframe scrolling="auto" frameborder="0"  src="webapp/scm/ori_exp.html?id='+id+'" style="width:100%;height:90%;overflow: scroll"></iframe>'
        }
    })
    tab.panel('refresh');
}
function historyReceipt() {
    addTopTab("#tabs",'其他支出单记录',"webapp/scm/billHistory.html?transType="+$("#transType").val());
}
...
function statistics() {
    ...
    if (dFooter) {
        ...
        $("#totalPayment").numberbox('setValue', isNaN(payment) ? 0 : payment);
    }
}
$(function () {
