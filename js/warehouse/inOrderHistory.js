...
function editPurchase() {
    var row = $("#inOrderRecords").datagrid('getSelections');
    if (row.length == 1) {
        var tabTitle = '其他入库单';
        var dg="#tabs";
        var url = "webapp/warehuse/inOrder.html?id="+row[0].id;
        addTopTab(dg,tabTitle,url);
        $.cookie('id',row[0].id);
        $("#inOrderRecords").datagrid("clearSelections");
    }else{
        layer.msg("请选中一行进行编辑");
        $("#inOrderRecords").datagrid("clearSelections");
        return false;
...
function auditPurchase() {
    var data = $("#inOrderRecords").datagrid('getSelections');
    var id="";
    ...
    $.ajax({
        ...
        success:function (res) {
            if(res.code==200){
                ...
                $("#inOrderRecords").datagrid("reload").datagrid("clearSelections");
